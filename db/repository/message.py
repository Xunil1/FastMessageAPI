...
async def add_message(chat_id: int, sender: str, msg: str, db: AsyncSession):
    ...
    message = Message(sender_id=s_user.id, chat_id=chat.id, message=msg)
    db.add(message)
    await db.commit()
...
async def get_history_by_chat(chat_id: id, sender: str, db: AsyncSession):
    result = await db.execute(select(Message).where(Message.chat_id == chat_id).order_by(Message.send_time))
